...
void collide(Grid_N_C_3D<T> &grid,
            lbmD3Q41<T1> &lb41, lbmD3Q15<T1> &lb15,real beta,real tau, real TbyTc, real kappa, int t,Grid_N_C_3D<T> &Force ){
    ...
    real rho_critical = 1.0, T_critical = lb41.theta0/TbyTc ; 
    real b = 0.521772/(rho_critical), a = b*T_critical/0.377332;
    kappa = kappa*a;

    // Multiphase_terms(grid,Force,rho,pnid, fnid, munid,laplacian_rho,laplacian_fnid,gradient_rho,lb41, lb15,TbyTc,kappa, a, b );
    ...
    for(int i = 0 + grid.noghost; i < grid.n_x_node - (grid.noghost) ; i++){
        for(int j = 0 + grid.noghost;j < grid.n_y_node - (grid.noghost) ; j++){
            for(int k = 0 + grid.noghost;k < grid.n_z_node - (grid.noghost) ; k++){
                ...
                // Multiphase_Force_Node(grid,rho,pnid, fnid, munid,laplacian_rho,lb41, lb15,Force,i,j,k, kappa, a, b );             										// $ chemical potential formulation
                // Multiphase_Force_eta_Node(grid,rho,pnid, fnid, munid,laplacian_rho,laplacian_fnid,gradient_rho,lb41,Fx,Fy,Fz,i,j,k,kappa, eta );  		// $41 paper
                
                // Force.Node(i,j,k,0) = 0;
                // Force.Node(i,j,k,1) = 0;
                // Force.Node(i,j,k,2) = 0;


                // get_moments_Node_force(grid, lb41,  ux, uy, uz,Rho, i, j, k,Force ,beta);            //for the node
                // get_equi(feq_Node_delu,lb41, ux, uy,uz, Rho);




                
                get_moments_Node(grid, lb41,  ux, uy, uz,Rho, i, j, k, Force );            //for the node
                get_equi(feq_Node ,lb41, ux, uy,uz, Rho);


                // //> normal
                for (int dv = 0; dv< grid.d_v; dv++){
                    grid.Node(i,j,k,dv) =  grid.Node(i,j,k,dv) + 2.0* beta*(feq_Node[dv] - grid.Node(i,j,k,dv))
                                        // + 2.0 *beta * tau*lb41.thetaInverse * rho.Node(i,j,k)* lb41.W[dv] * (Fx * lb41.Cx[dv] + Fy * lb41.Cy[dv] + Fz * lb41.Cz[dv])
                                        ;
                }
                ...
                //         calculateAlpha(lb41,x_i,&grid.Node(i,j,0),beta,alpha);
                //         break;
                //     }
                // }
                ...
                // //                         + (1 - 0.5*alpha*beta)*lb41.thetaInverse * feq_Node[dv] * (Force.Node(i,j,k,0) * (lb41.Cx[dv] - ux) + Force.Node(i,j,k,1) * (lb41.Cy[dv] - uy) + Force.Node(i,j,k,2) * (lb41.Cz[dv]- uz) );
                // // }   
                ...
                // Multiphase_Force_Cell(grid,rho,pnid, fnid, munid,laplacian_rho,lb41,lb15,Force,i,j,k, kappa, a ,b );                                               // $ chemical potential
                // Multiphase_Force_eta_Cell(grid,rho,pnid, fnid, munid,laplacian_rho,laplacian_fnid,gradient_rho,lb41,Fx,Fy,Fz,i,j,k,kappa,eta );  // $ 41 paper


                // Force.Cell(i,j,k,0) = 0;
                // Force.Cell(i,j,k,1) = 0;
                // Force.Cell(i,j,k,2) = 0;


                // get_moments_Cell_force(grid, lb41,  ux, uy, uz,Rho, i, j, k,Force, beta);            //for the cell
                // get_equi(feq_Cell_delu,lb41, ux, uy,uz, Rho);




                
                get_moments_Cell(grid, lb41,  ux, uy, uz,Rho, i, j, k,Force);            //for the cell
                get_equi(feq_Cell,lb41, ux, uy,uz, Rho);
                ...
                for (int dv = 0; dv< grid.d_v; dv++){
                    grid.Cell(i,j,k,dv) =  grid.Cell(i,j,k,dv) + 2.0* beta*(feq_Cell[dv] - grid.Cell(i,j,k,dv))
                                        // + 2.0 *beta * tau*lb41.thetaInverse * rho.Cell(i,j,k)* lb41.W[dv] * (Fx * lb41.Cx[dv] + Fy * lb41.Cy[dv] + Fz * lb41.Cz[dv])
                                        ;
                }       
                ...
                //     alpha = 2.0;
                //     if( std::fabs(x_i[dv]) > 0.0001){
                //         calculateAlpha(lb41,x_i,&grid.Cell(i,j,0),beta,alpha);
                //         break;
                //     }
                // }
                ...
                // //                         + (1 - 0.5*alpha*beta)*lb41.thetaInverse * feq_Cell[dv] * (Force.Cell(i,j,k,0) * (lb41.Cx[dv]- ux) +Force.Cell(i,j,k,1)* (lb41.Cy[dv]-uy) + Force.Cell(i,j,k,2) *( lb41.Cz[dv] - uz) );
                // // }


                // for (int dv = 0; dv< 41; dv++){
                //     grid.Cell(i,j,k,dv) =  grid.Cell(i,j,k,dv) + alpha* beta*(feq_Cell[dv] - grid.Cell(i,j,k,dv))
                //                         // + (feq_Cell_delu[dv] - feq_Cell[dv])
                //                         ;
                // }
...
template<typename T>
void calculateAlpha(lbmD3Q41<T> &lbModel,T* x_i,T* f_i,T beta,T& alpha)
{
...
template<typename T>
void get_equi(real *feq , lbmD3Q41<T> &lb, real ux, real uy, real uz, real rho){
...
template<typename T,typename T1>
void get_moments_Node(Grid_N_C_3D<T> &grid, lbmD3Q41<T1> &lb,real &Ux, real &Uy, real &Uz,real &Rho,  int X, int Y, int Z, Grid_N_C_3D<T> &Force){ ///node or cell 0-Node 1- cell
    Ux  = 0.0;
    Uy  = 0.0;
    Uz  = 0.0;
    Rho = 0.0;
...
template<typename T,typename T1>
void get_moments_Node_force(Grid_N_C_3D<T> &grid, lbmD3Q41<T1> &lb,real &Ux, real &Uy, real &Uz,real &Rho,  int X, int Y, int Z, Grid_N_C_3D<T> &Force, real beta){ ///node or cell 0-Node 1- cell
    Ux  = 0.0;
    Uy  = 0.0;
    Uz  = 0.0;
    Rho = 0.0;
...
template<typename T,typename T1>
void get_moments_Cell(Grid_N_C_3D<T> &grid, lbmD3Q41<T1> &lb,real &Ux, real &Uy, real &Uz,real &Rho,  int X, int Y, int Z,Grid_N_C_3D<T> &Force){ ///node or cell 0-Cell 1- cell
    Ux  = 0.0;
    Uy  = 0.0;
    Uz  = 0.0;
    Rho = 0.0;
...
template<typename T,typename T1>
void get_moments_Cell_force(Grid_N_C_3D<T> &grid, lbmD3Q41<T1> &lb,real &Ux, real &Uy, real &Uz,real &Rho,  int X, int Y, int Z,Grid_N_C_3D<T> &Force, real beta){ ///node or cell 0-Cell 1- cell
    Ux  = 0.0;
    Uy  = 0.0;
    Uz  = 0.0;
    Rho = 0.0;
...
//period no of waves
template<typename T, typename T1>
void initialization(Grid_N_C_3D<T> &grid,lbmD3Q41<T1> &lb,real Rho_mean ,real amplitude, real period){


    real Feq_node[41] = {0},Feq_cell[41] = {0},Rho = 0.0;
    real x,y,z
...
template<typename T, typename T1>
void initialization_equilibrium_profile_x(Grid_N_C_3D<T> &grid,lbmD3Q41<T1> &lb,real Rho_mean ){
...
template<typename T, typename T1>
void initialization_equilibrium_profile_y(Grid_N_C_3D<T> &grid,lbmD3Q41<T1> &lb,real rho_liq,real rho_gas ){
...
template<typename T, typename T1>
void initialization_2D_droplet(Grid_N_C_3D<T> &grid,lbmD3Q41<T1> &lb,real Rho_mean ){
