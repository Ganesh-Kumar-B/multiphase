...
void printdata(lbmD3Q41<T1> &lbModel,  Grid_N_C_3D<T> &gridLB,  int step, real u0)
{
...
template<typename T, typename T1>
void print_vtk(lbmD3Q41<T1> &lb,  Grid_N_C_3D<T> &grid,  int step, real u0, real theta,real kappa, Grid_N_C_3D<T> &Force)
{
    T u1,u2,u3,u4,um, rho1,rho2,del=0.05;
    ...
    char fileName[250];
    char foldername[250];
    sprintf(foldername,"Non_entropic_%0.6f_%.2f",kappa,theta);
    mkdir(foldername,S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    sprintf(fileName,"./Non_entropic_%0.6f_%.2f/velocity_%d.vtk",kappa, theta,step) ;
    file.open(fileName);
    // file<<"# vtk DataFile Version 3.0\nVelocity\nASCII\nDATASET STRUCTURED_GRID"<<std::endl;
    file<<"# vtk DataFile Version 3.0\nVelocity\nASCII\nDATASET STRUCTURED_POINTS"<<std::endl;
    
    file<<"DIMENSIONS "<<1*grid.n_x<<" "<<1*grid.n_y<<" "<<1*grid.n_z<<std::endl;
    ...
    for(int k = 0 + grid.noghost; k < grid.n_z_node - (grid.noghost); k++){
	    for (int j = 0 + grid.noghost; j < grid.n_y_node - (grid.noghost); j++){
            for(int i = 0 + grid.noghost; i < grid.n_x_node - (grid.noghost); i++){ 
                ...
                file<<u1/u0<<" "<<u2/u0<<" "<<u3/u0<<std::endl;
