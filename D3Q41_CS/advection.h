    ...
    for(int i = 0 + grid.noghost ; i < grid.n_x_node - (grid.noghost);i++){
        for(int j = 0 + grid.noghost ; j < grid.n_y_node - (grid.noghost);j++){ 
            for(int k = 0 + grid.noghost ; k < grid.n_z_node - (grid.noghost);k++){ 
                ...
                grid.Node(i,j,k,  dV_M1_M1_M1   )           = grid.Node(i +1,j +1,k +1,  dV_M1_M1_M1   ) ;
                grid.Node(i,j,k,  dV_M1_P1_M1   )           = grid.Node(i +1,j -1,k +1,  dV_M1_P1_M1   ) ;
                grid.Node(i,j,k,  dV_M1_M1_P1   )           = grid.Node(i +1,j +1,k -1,  dV_M1_M1_P1   ) ;
                grid.Node(i,j,k,  dV_M1_P1_P1   )           = grid.Node(i +1,j -1,k -1,  dV_M1_P1_P1   ) ;
                ...
                grid.Cell(i,j,k,  dV_M1_ZERO_ZERO   )     = grid.Cell(i+1,j  ,k  , dV_M1_ZERO_ZERO ); 
                grid.Cell(i,j,k,  dV_ZERO_M1_ZERO   )     = grid.Cell(i  ,j+1,k  , dV_ZERO_M1_ZERO ); 
                grid.Cell(i,j,k,  dV_ZERO_ZERO_M1   )     = grid.Cell(i  ,j  ,k+1, dV_ZERO_ZERO_M1 );
                ...
                grid.Cell(i,j,k,  dV_M1_M1_M1   )         = grid.Cell(i +1,j +1,k +1,  dV_M1_M1_M1   )      ;
                grid.Cell(i,j,k,  dV_M1_P1_M1   )         = grid.Cell(i +1,j -1,k +1,  dV_M1_P1_M1   )      ;
                grid.Cell(i,j,k,  dV_M1_M1_P1   )         = grid.Cell(i +1,j +1,k -1,  dV_M1_M1_P1   )      ;
                grid.Cell(i,j,k,  dV_M1_P1_P1   )         = grid.Cell(i +1,j -1,k -1,  dV_M1_P1_P1   )      ;
    ...
    for(int i = grid.n_x_node - (grid.noghost +1); i> grid.noghost-1; i--){                                                                                        //  #pragma omp parallel for shared(b)
        for(int j = grid.n_y_node - (grid.noghost +1); j> grid.noghost-1; j--){
            for(int k = grid.n_z_node - (grid.noghost +1); k> grid.noghost-1; k--){
                ...
                grid.Node(i,j,k,dV_P1_P1_P1  )      = grid.Node(i -1,j -1,k -1,dV_P1_P1_P1  ) ;
                grid.Node(i,j,k,dV_P1_M1_P1  )      = grid.Node(i -1,j +1,k -1,dV_P1_M1_P1  ) ;
                grid.Node(i,j,k,dV_P1_P1_M1  )      = grid.Node(i -1,j -1,k +1,dV_P1_P1_M1  ) ;
                grid.Node(i,j,k,dV_P1_M1_M1  )      = grid.Node(i -1,j +1,k +1,dV_P1_M1_M1  ) ;
        


                //>
                grid.Cell(i,j,k,dV_P1_ZERO_ZERO   ) = grid.Cell(i-1,j  ,k  , dV_P1_ZERO_ZERO );      
                grid.Cell(i,j,k,dV_ZERO_P1_ZERO   ) = grid.Cell(i  ,j-1,k  , dV_ZERO_P1_ZERO );      
                grid.Cell(i,j,k,dV_ZERO_ZERO_P1   ) = grid.Cell(i  ,j  ,k-1, dV_ZERO_ZERO_P1 );
                ...
                grid.Cell(i,j,k,dV_P1_P1_P1  ) = grid.Cell(i -1,j -1,k -1,dV_P1_P1_P1  ) ;
                grid.Cell(i,j,k,dV_P1_M1_P1  ) = grid.Cell(i -1,j +1,k -1,dV_P1_M1_P1  ) ;
                grid.Cell(i,j,k,dV_P1_P1_M1  ) = grid.Cell(i -1,j -1,k +1,dV_P1_P1_M1  ) ;
                grid.Cell(i,j,k,dV_P1_M1_M1  ) = grid.Cell(i -1,j +1,k +1,dV_P1_M1_M1  ) ;
