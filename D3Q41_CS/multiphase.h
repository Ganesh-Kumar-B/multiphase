...
void Multiphase_terms(Grid_N_C_3D<T> &grid, Grid_N_C_3D<T> &Force, Grid_N_C_3D<T> &rho, Grid_N_C_3D<T> &pnid, Grid_N_C_3D<T> &fnid, Grid_N_C_3D<T> &munid, Grid_N_C_3D<T> &laplacian_rho,  Grid_N_C_3D<T> &laplacian_fnid, Grid_N_C_3D<T> &gradient_rho, 
            lbmD3Q41<T1> &lb41,lbmD3Q15<T1> &lb15, real TbyTc, real kappa, real a , real b ){
    ...
    for(int i = 0 + grid.noghost; i < grid.n_x_node - (grid.noghost) ; i++){
        for(int j = 0 + grid.noghost;j < grid.n_y_node - (grid.noghost) ; j++){
            for(int k = 0 + grid.noghost;k < grid.n_z_node - (grid.noghost) ; k++){
                ...
                get_moments_Node(grid, lb41,  ux, uy, uz, rho.Node(i,j,k), i, j ,k, Force); 

                get_moments_Cell(grid, lb41,  ux, uy, uz, rho.Cell(i,j,k), i, j ,k, Force); 

                real eta = rho.Node(i,j,k)*b/4.0;

                //> pnid Node
                pnid.Node(i,j,k) = (rho.Node(i,j,k)*lb41.theta0*(1.0 + eta + eta* eta - eta*eta*eta) )/pow(1.0 - eta,  3.0)  - 
                                    a * rho.Node(i,j,k)*rho.Node(i,j,k) ;

                //> Fnid_node
                fnid.Node(i,j,k) = -1.0*(rho.Node(i,j,k)*lb41.theta0*(3.0*eta*eta - 4.0 * eta) )/pow(1.0 - eta,  2.0)  - 
                                    a * rho.Node(i,j,k)*rho.Node(i,j,k) ;

                eta  = rho.Cell(i,j,k)*b/4.0;
                //> pnid Cell
                pnid.Cell(i,j,k) = (rho.Cell(i,j,k)*lb41.theta0*(1.0 + eta + eta* eta - eta*eta*eta) )/pow(1.0 - eta,  3.0)  - 
                                    a * rho.Cell(i,j,k)*rho.Cell(i,j,k) ;

                //>Fnid_Cell
                fnid.Cell(i,j,k) =  -1.0*(rho.Cell(i,j,k)*lb41.theta0*(3.0*eta*eta - 4.0 * eta) )/pow(1.0 - eta,  2.0)  - 
                                    a * rho.Cell(i,j,k)*rho.Cell(i,j,k) ;
    ...
    for(int i = 0 + grid.noghost; i < grid.n_x_node - (grid.noghost) ; i++){
        for(int j = 0 + grid.noghost;j < grid.n_y_node - (grid.noghost) ; j++){
            for(int k = 0 + grid.noghost;k < grid.n_z_node - (grid.noghost) ; k++){
                ...
                //> laplacian of Rho  
                real del_t = 1.0;
                real Coeff = (2.0/(del_t*del_t*lb41.theta0));
                
                laplacian_rho.Node(i,j,k)  = 0.0;

                for(int dv = 0; dv< 27; dv++)
                    laplacian_rho.Node(i,j,k) += lb41.W[dv]*rho.Node( i+ (int)lb41.Cx[dv]  , j + (int)lb41.Cy[dv] , k + (int)lb41.Cz[dv] ) ;

                for(int dv = 27; dv< 41;dv++)
                    laplacian_rho.Node(i,j,k) += lb41.W[dv]*rho.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]) ;
                ...
                for(int dv = 0; dv< 27; dv++)
                    laplacian_rho.Cell(i,j,k) += lb41.W[dv]*rho.Cell( i+ (int)lb41.Cx[dv]  , j + (int)lb41.Cy[dv] , k + (int)lb41.Cz[dv] ) ;

                for(int dv = 27; dv< 41;dv++)
                    laplacian_rho.Cell(i,j,k) += lb41.W[dv]*rho.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]) ;
                ...
                // kappa_node = kappa - (1.0/2.0)* lb41.theta0* (
                //                                         -32.0 *b* lb41.theta0*(-16.0 + b* rho.Node(i,j,k)) / (pow(-4.0 + b*rho.Node(i,j,k) , 4.0))
                //                                             - 2.0*a
                //                                         ) 
                //                                     ;
                ...
                munid.Node(i,j,k) += lb41.theta0*(3.0*eta*eta*eta - 9.0*eta*eta + 8.0*eta) / pow(1.0 - eta, 3.0);
                ...
                // kappa_cell = kappa - (1.0/2.0)* lb41.theta0* (
                //                                        -32.0 *b* lb41.theta0*(-16.0 + b* rho.Cell(i,j,k)) / (pow(-4.0 + b*rho.Cell(i,j,k) , 4.0))
                //                                             - 2.0*a
                //                                         ) 
                //                                     ;
                ...
                munid.Cell(i,j,k) += lb41.theta0*(3.0*eta*eta*eta - 9.0*eta*eta + 8.0*eta) / pow(1.0 - eta, 3.0);
...
void Multiphase_Force_Node(Grid_N_C_3D<T> &grid, Grid_N_C_3D<T> &rho, Grid_N_C_3D<T> &pnid, Grid_N_C_3D<T> &fnid, Grid_N_C_3D<T> &munid, Grid_N_C_3D<T> &laplacian_rho,
            lbmD3Q41<T1> &lb41,lbmD3Q15<T1> &lb15, Grid_N_C_3D<T> &Force, int i, int j,int k, real kappa, real a , real b){
    ...
    real del_t = 1.0;
    real Coeff_grad = (1.0/(del_t*lb41.theta0));

    for(int dv = 0; dv< 27; dv++){
        grad_mux += lb41.W[dv]*lb41.Cx[dv]*munid.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) ;
        grad_muy += lb41.W[dv]*lb41.Cy[dv]*munid.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) ;
        grad_muz += lb41.W[dv]*lb41.Cz[dv]*munid.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) ;
    }

    for(int dv = 27; dv<grid.d_v; dv++){
        grad_mux += lb41.W[dv]*lb41.Cx[dv]*munid.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]) ;
        grad_muy += lb41.W[dv]*lb41.Cy[dv]*munid.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]) ;
        grad_muz += lb41.W[dv]*lb41.Cz[dv]*munid.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]) ;
    }
    ...
    //                         lb41.W[dv]*lb41.Cx[dv]*( (pnid.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) + fnid.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]))/ rho.Node( i+ (int)lb41.Cx[dv] ,j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])  )
    //                         )
    //                         - kappa_node*Coeff_grad*( lb41.W[dv]*lb41.Cx[dv]*( laplacian_rho.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])))
    //                         ;

    //     Force.Node(i,j,k,1) += Coeff_grad*(               
    //                         lb41.W[dv]*lb41.Cy[dv]*( (pnid.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) + fnid.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]))/ rho.Node( i+ (int)lb41.Cx[dv] ,j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])  )
    //                         )
    //                         - kappa_node*Coeff_grad*( lb41.W[dv]*lb41.Cy[dv]*( laplacian_rho.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])))
    //                         ;

    //     Force.Node(i,j,k,2) += Coeff_grad*(               
    //                         lb41.W[dv]*lb41.Cz[dv]*( (pnid.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) + fnid.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]))/ rho.Node( i+ (int)lb41.Cx[dv] ,j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])  )
    //                         )
    //                         - kappa_node*Coeff_grad*( lb41.W[dv]*lb41.Cz[dv]*( laplacian_rho.Node( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])))
    //                         ;
    ...
    //     Force.Node(i,j,k,0) += Coeff_grad*(               
    //                         lb41.W[dv]*lb41.Cx[dv]*( (pnid.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]) + fnid.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]))/ rho.Cell( i+ (int)lb41.CxF[dv] ,j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv])  )
    //                         )
    //                         - kappa_node*Coeff_grad*( lb41.W[dv]*lb41.Cx[dv]*( laplacian_rho.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv])))
    //                         ;

    //     Force.Node(i,j,k,1) += Coeff_grad*(               
    //                         lb41.W[dv]*lb41.Cy[dv]*( (pnid.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]) + fnid.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]))/ rho.Cell( i+ (int)lb41.CxF[dv] ,j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv])  )
    //                         )
    //                         - kappa_node*Coeff_grad*( lb41.W[dv]*lb41.Cy[dv]*( laplacian_rho.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv])))
    //                         ;

    //     Force.Node(i,j,k,2) += Coeff_grad*(               
    //                         lb41.W[dv]*lb41.Cz[dv]*( (pnid.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]) + fnid.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv]))/ rho.Cell( i+ (int)lb41.CxF[dv] ,j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv])  )
    //                         )
    //                         - kappa_node*Coeff_grad*( lb41.W[dv]*lb41.Cz[dv]*( laplacian_rho.Cell( i+ (int)lb41.CxF[dv] , j + (int)lb41.CyF[dv], k + (int)lb41.CzF[dv])))
    //                         ;
    ...
    // // real coeff_4th_grad = (lb41.theta0*del_t*del_t)/2.0;

    // // for(int dv = 0; dv < grid.d_v; dv++){
    // //     Force.Node(i,j,k,0) -= coeff_4th_grad* Coeff_grad*(lb41.W[dv]*lb41.Cx[dv]*laplacian_pnidplusfnidbyrho.Node( i+ lb41.Cx[dv] , j + lb41.Cy[dv],k + lb41.Cz[dv])) ;
    // //     Force.Node(i,j,k,1) -= coeff_4th_grad* Coeff_grad*(lb41.W[dv]*lb41.Cy[dv]*laplacian_pnidplusfnidbyrho.Node( i+ lb41.Cx[dv] , j + lb41.Cy[dv],k + lb41.Cz[dv])) ;
    // //     Force.Node(i,j,k,2) -= coeff_4th_grad* Coeff_grad*(lb41.W[dv]*lb41.Cz[dv]*laplacian_pnidplusfnidbyrho.Node( i+ lb41.Cx[dv] , j + lb41.Cy[dv],k + lb41.Cz[dv])) ;
    // // }
...
void Multiphase_Force_Cell(Grid_N_C_3D<T> &grid, Grid_N_C_3D<T> &rho, Grid_N_C_3D<T> &pnid, Grid_N_C_3D<T> &fnid, Grid_N_C_3D<T> &munid, Grid_N_C_3D<T> &laplacian_rho,
    ...
    //> CHEMICAL POTENTIAL FORMULATION 
    del_t = 1.0;
    Coeff_grad = (1.0/(del_t*lb41.theta0));

    for(int dv = 0; dv< 27; dv++){
        grad_mux += lb41.W[dv]*lb41.Cx[dv]*munid.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) ;
        grad_muy += lb41.W[dv]*lb41.Cy[dv]*munid.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) ;
        grad_muz += lb41.W[dv]*lb41.Cz[dv]*munid.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) ;
    }

    for(int dv = 27; dv< 41; dv++){
        grad_mux += lb41.W[dv]*lb41.Cx[dv]*munid.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]) ;
        grad_muy += lb41.W[dv]*lb41.Cy[dv]*munid.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]) ;
        grad_muz += lb41.W[dv]*lb41.Cz[dv]*munid.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]) ;
    }


    Force.Cell(i,j,k,0) = - Coeff_grad*(grad_mux);
    Force.Cell(i,j,k,1) = - Coeff_grad*(grad_muy) - 0.0001;  
...
//                             lb41.W[dv]*lb41.Cx[dv]*( (pnid.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) + fnid.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]))/ rho.Cell( i+ (int)lb41.Cx[dv] ,j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])  )
//                             )
//                             - kappa_cell*Coeff_grad*( lb41.W[dv]*lb41.Cx[dv]*( laplacian_rho.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])))
//                             ;

//             Force.Cell(i,j,k,1) += Coeff_grad*(               
//                             lb41.W[dv]*lb41.Cy[dv]*( (pnid.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) + fnid.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]))/ rho.Cell( i+ (int)lb41.Cx[dv] ,j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])  )
//                             )
//                             - kappa_cell*Coeff_grad*( lb41.W[dv]*lb41.Cy[dv]*( laplacian_rho.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])))
//                             ;

//             Force.Cell(i,j,k,2) += Coeff_grad*(               
//                             lb41.W[dv]*lb41.Cz[dv]*( (pnid.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]) + fnid.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv]))/ rho.Cell( i+ (int)lb41.Cx[dv] ,j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])  )
//                             )
//                             - kappa_cell*Coeff_grad*( lb41.W[dv]*lb41.Cz[dv]*( laplacian_rho.Cell( i+ (int)lb41.Cx[dv] , j + (int)lb41.Cy[dv], k + (int)lb41.Cz[dv])))
//                             ;
...
//             Force.Cell(i,j,k,0) += Coeff_grad*(               
//                             lb41.W[dv]*lb41.Cx[dv]*( (pnid.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]) + fnid.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]))/ rho.Node( i+ (int)lb41.CxC[dv] ,j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv])  )
//                             )
//                             - kappa_cell*Coeff_grad*( lb41.W[dv]*lb41.Cx[dv]*( laplacian_rho.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv])))
//                             ;

//             Force.Cell(i,j,k,1) += Coeff_grad*(               
//                             lb41.W[dv]*lb41.Cy[dv]*( (pnid.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]) + fnid.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]))/ rho.Node( i+ (int)lb41.CxC[dv] ,j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv])  )
//                             )
//                             - kappa_cell*Coeff_grad*( lb41.W[dv]*lb41.Cy[dv]*( laplacian_rho.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv])))
//                             ;

//             Force.Cell(i,j,k,2) += Coeff_grad*(               
//                             lb41.W[dv]*lb41.Cz[dv]*( (pnid.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]) + fnid.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv]))/ rho.Node( i+ (int)lb41.CxC[dv] ,j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv])  )
//                             )
//                             - kappa_cell*Coeff_grad*( lb41.W[dv]*lb41.Cz[dv]*( laplacian_rho.Node( i+ (int)lb41.CxC[dv] , j + (int)lb41.CyC[dv], k + (int)lb41.CzC[dv])))
//                             ;
...
//     // // //> 4th order corrections
//     // //  coeff_4th_grad = (lb41.theta0*del_t*del_t)/2.0;

//     // // for(int dv = 0; dv < grid.d_v; dv++){
//     // //     Force.Cell(i,j,k,0) -= coeff_4th_grad* Coeff_grad*(lb41.W[dv]*lb41.Cx[dv]*laplacian_pnidplusfnidbyrho.Cell( i+ lb41.Cx[dv] , j + lb41.Cy[dv],k + lb41.Cz[dv])) ;
//     // //     Force.Cell(i,j,k,1) -= coeff_4th_grad* Coeff_grad*(lb41.W[dv]*lb41.Cy[dv]*laplacian_pnidplusfnidbyrho.Cell( i+ lb41.Cx[dv] , j + lb41.Cy[dv],k + lb41.Cz[dv])) ;
//     // //     Force.Cell(i,j,k,2) -= coeff_4th_grad* Coeff_grad*(lb41.W[dv]*lb41.Cz[dv]*laplacian_pnidplusfnidbyrho.Cell( i+ lb41.Cx[dv] , j + lb41.Cy[dv],k + lb41.Cz[dv])) ;
//     // // }
...
void Multiphase_Force_eta_Node(Grid_N_C_3D<T> &grid, Grid_N_C_3D<T> &rho, Grid_N_C_3D<T> &pnid, Grid_N_C_3D<T> &fnid, Grid_N_C_3D<T> &munid, Grid_N_C_3D<T> &laplacian_rho,  Grid_N_C_3D<T> &laplacian_fnid, Grid_N_C_3D<T> &gradient_rho, 
            lbmD3Q41<T1> &lb, real &Fx, real &Fy, real &Fz, int i, int j,int k, real kappa, real eta){
