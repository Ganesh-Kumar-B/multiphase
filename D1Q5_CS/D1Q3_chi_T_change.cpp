...
main()
{
	...
	initializePerturbPeriodic(  myLattice,   myD1Q3,    nX, 0.0, myVDW.rho0, 0.0001,2 );//0.001,2
...
void printRho(latticeArr myLattice, latticeD1Q3 myD1Q3, nonIdealParam myVDW, int nX, double beta,int iX_begin, int iX_end, int time, double c)
{
    ...
    for(iX =iX_begin; iX<iX_end;iX++)
    {
        ...
        P =(myLattice[iX].rho*myD1Q3.T0  ) *(1+eta_EOS + eta_EOS*eta_EOS - pow(eta_EOS, 3) )/pow((1.0 - eta_EOS), 3)
