...
void print_vtk(lbmD2Q9<T1> &lb,  Grid_N_C_2D<T> &grid,  int step, real u0, real theta,real kappa,real a, real b, Grid_N_C_2D<T> &Force,Grid_N_C_2D<T> &P_tensor, const std::string &name, real dx, real dt)
{
    ...
    for (int j = 0 + grid.noghost; j < grid.n_y_node - (grid.noghost); j++){
        ...
        int i = grid.n_x/2; int j = grid.n_y/2;
