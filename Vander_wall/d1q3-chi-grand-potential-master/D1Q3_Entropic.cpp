...
typedef  struct nodeD1Q3
{
    ...
    double rhoOld;
    double pNid;
    double pNidPredictor;
    double pNidCorrector;
    double chiQmPu;
    double muR;
    double muA;
    double dmuA;
...
void initializeGrandPotential(latticeArr myLattice, latticeD1Q3 myD1Q3,  nonIdealParam myVDW, int nX, double vel, double rhoMean, double ampDisturb, double periodDisturb)
{
    ...
    for(iX =3; iX<=nX+2;iX++)
    {
        ...
        double pNid = rho*rho*myVDW.b*myD1Q3.T0/(1.0-myLattice[iX].rho*myVDW.b) - myVDW.a*rho*rho;
        myLattice[iX].pNid = pNid;
    }
...
//     dt *= 0.1;
    ...
    {
       double Peqnid = myLattice[iX].rho*myLattice[iX].rho*myVDW.b*myD1Q3.T0/(1-myLattice[iX].rho*myVDW.b) - myVDW.a*myLattice[iX].rho*myLattice[iX].rho;

//        double Qold  = (1+dtBy2tau)*myLattice[iX].pNid - dtBy2tau*Peqnid;
//        double Qnext = (1-twoBeta)*Qold + twoBeta*Peqnid;
//        myLattice[iX].pNid = oneMinusdtBy2tau*myLattice[iX].pNid + dtBy2tau*Peqnid ;
       
//        myLattice[iX].pNid = (1-twoBeta)*myLattice[iX].pNid + twoBeta*Peqnid;
       myLattice[iX].pNid = (myLattice[iX].pNid + dtBy2tau*Peqnid)/(1+dtBy2tau) ;
    }
	myLattice[0     ].pNid  = myLattice[nX     ].pNid ;
    myLattice[1     ].pNid  = myLattice[nX + 1 ].pNid ;
    myLattice[2     ].pNid  = myLattice[nX + 2 ].pNid ;
...
//     dt *= 0.1;
    ...
    // // // Predictor // // //
    for( int iX = nX+2  ; iX >=3 ; iX--)
       myLattice[iX].pNidPredictor = myLattice[iX].pNid - dt/dx*myLattice[iX].vel*( myLattice[iX+1].pNid - myLattice[iX].pNid ) - dt/dx*myLattice[iX].pNid*( myLattice[iX+1].vel - myLattice[iX].vel );
	myLattice[0     ].pNidPredictor  = myLattice[nX     ].pNidPredictor ;
    myLattice[1     ].pNidPredictor  = myLattice[nX + 1 ].pNidPredictor ;
    myLattice[2     ].pNidPredictor  = myLattice[nX + 2 ].pNidPredictor ;
    myLattice[nX + 3].pNidPredictor  = myLattice[3      ].pNidPredictor ;
    myLattice[nX + 4].pNidPredictor  = myLattice[4      ].pNidPredictor ;
    myLattice[nX + 5].pNidPredictor  = myLattice[5      ].pNidPredictor ;

    // // // Corrector // // //
    for( int iX = nX+2  ; iX >=3 ; iX--)
       myLattice[iX].pNidCorrector = 0.5*( myLattice[iX].pNidPredictor + myLattice[iX].pNid ) - 0.5*dt/dx*myLattice[iX].vel*( myLattice[iX].pNidPredictor - myLattice[iX-1].pNidPredictor ) - 0.5*dt/dx*myLattice[iX].pNidPredictor*( myLattice[iX].vel - myLattice[iX-1].vel ) ;

          
    for( int iX = nX+2  ; iX >=3 ; iX--) 
//        myLattice[iX].pNid = 0.5*(myLattice[iX].pNidCorrector + myLattice[iX].pNidPredictor);
//        myLattice[iX].pNid = myLattice[iX].pNidPredictor ;
       myLattice[iX].pNid = myLattice[iX].pNidCorrector ;
...
void printRho(latticeArr myLattice, latticeD1Q3 myD1Q3, nonIdealParam myVDW, int nX, double beta,int iX_begin, int iX_end, int time, double c)
{
    ...
    sprintf(fileName,"rho%d.dat", time);
    ...
    for(iX =iX_begin; iX<iX_end;iX++)
    {
        ...
        fprintf(fpt,  "%d %.7lf  %.7lf %.7lf  %.7lf \n",iX, myLattice[iX].rho, myLattice[iX].vel, P, mu);
    }
    fclose(fpt);
}
...
//         myLattice[iX].Force = myLattice[iX].rho*(myLattice[iX+1].muA -myLattice[iX-1].muA)/(dx*2.0);
        ...
        myLattice[iX].Force = -myLattice[iX].Force/myLattice[iX].rho;
